 ...
 *  //> undefined
 */
 function find(array, callback) { //placeholder
  for (let element of array) { // checking index of array
    if (callback(element)){ // callbacks are arrow functions
        return element; 
    }
  }
    //return undifined;
...
function filter(array, callback) {
  ...
  for (let element of array) { // for loop
    if(callback(element)){  // 
      result.push(element) // pushing elememnt into result array
    }
  }
  return result;
}
